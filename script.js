...
document.querySelector("form").addEventListener("submit", function (e) {
    ...
    if (isValidInput(floorCount, liftCount)) {
        ...
        alert(
            "Please enter valid numbers for floors and lifts. The number of lifts should not exceed the number of floors.",
        );
    }
});
...
function handleButtonClicks() {
    document.querySelectorAll(".up, .down, .call").forEach(function (button) {
        // selects all elements with "up" "down" "call" classname and iterates for each selected element
        button.addEventListener("click", function () {
            //adds "click" event listner to each button
            ...
            const existingLift = liftData.find(
                (lift) => lift.targetFloor === floorID && lift.direction === buttonType,
            );
...
function processLiftRequests() {
    ...
    if (availableLift !== null) {
        // if available lift is found
        buttonPressQueue.shift(); // Remove the request only if it can be processed
        moveLift(availableLift, request.floorID, request.buttonType); // calls the moveLift function with (liftIndex of the available lift, floorID of the button press request)
    } else {
    ...
    liftData.forEach(function (lift) {
        if (!lift.isMoving) {
            ...
            if (distance < minDistance) {
                // If the current lift is closer than the previous nearest lift
...
function moveLift(lift, targetFloor, direction) {
    ...
    setTimeout(() => {
        ...
        animateDoors(liftElement, () => {
            // animateDoors() function is called to visually animate the lift doors
